...
class RedisTaskQueueV2:
    ...
    def get_task(self, worker_id: str, timeout: int = 5) -> Optional[Dict]:
        ...
        pipe.hset(f"task:{task_id}", "worker_id", worker_id)
        pipe.hset(f"task:{task_id}", "started_at", datetime.utcnow().isoformat())
        pipe.execute()
    ...
    def mark_completed(self, task_id: str, result: Optional[Dict] = None):
        ...
        pipe.hset(f"task:{task_id}", "completed_at", datetime.utcnow().isoformat())
        if result:
            pipe.hset(f"task:{task_id}", "result", json.dumps(result))
    ...
    def mark_failed(
        ...
        # Actualizar metadata
        pipe = self.redis.pipeline()
        pipe.hset(f"task:{task_id}", "retry_count", retry_count)
        pipe.hset(f"task:{task_id}", "last_error", error_message)
        pipe.hset(f"task:{task_id}", "failed_at", datetime.utcnow().isoformat())
        ...
        if should_retry and retry_count < self.max_retries:
            ...
            pipe.hset(f"task:{task_id}", "status", "dead")
