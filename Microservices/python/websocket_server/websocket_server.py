#!/usr/bin/env python3
"""
Servidor WebSocket para manejo de notificaciones de mesas en tiempo real
...
async def register_client(websocket):
    ...
    welcome_message = {
        "type": "connection",
        "message": "Conectado al servidor de notificaciones de mesas",
    ...
    try:
        ...
        async for message in websocket:
            try:
                ...
                if data.get("type") == "desk_created":
                    desk = data.get("desk", {})
                    notification = {
                        "type": "desk_notification",
                        "action": "created",
                        "desk": desk,
                        "message": f"🪑 Nueva mesa creada: {desk.get('name', 'Sin nombre')} ({desk.get('width')}x{desk.get('height')}cm)",
                        "timestamp": datetime.utcnow().isoformat() + 'Z'
                    }
                    await broadcast_message(notification)
                
                elif data.get("type") == "desk_updated":
                    desk = data.get("desk", {})
                    notification = {
                        "type": "desk_notification",
                        "action": "updated",
                        "desk": desk,
                        "message": f"🔄 Mesa actualizada: {desk.get('name', 'Sin nombre')} - {desk.get('width')}x{desk.get('height')}cm",
                        "timestamp": datetime.utcnow().isoformat() + 'Z'
                    }
                    await broadcast_message(notification)
                
                elif data.get("type") == "desk_deleted":
                    desk = data.get("desk", {})
                    notification = {
                        "type": "desk_notification",
                        "action": "deleted",
                        "desk": desk,
                        "message": f"🗑️ Mesa eliminada: {desk.get('name', 'Sin nombre')}",
                        "timestamp": datetime.utcnow().isoformat() + 'Z'
                    }
                    await broadcast_message(notification)
...
async def main():
    ...
    logger.info("🚀 Iniciando servidor WebSocket para Mesas...")
